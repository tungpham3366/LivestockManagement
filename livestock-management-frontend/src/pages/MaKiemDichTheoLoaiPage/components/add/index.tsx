...
export const Add = () => {
  ...
  return (
    <Card className="mx-auto w-full max-w-md">
      <CardHeader>
        <CardTitle className="text-center">Tạo mới mã kiểm dịch</CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="space-y-4">
          <h3 className="font-medium">Chọn loại vật nuôi:</h3>
          <div className="space-y-3">{renderSpeciesCheckboxes()}</div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label htmlFor="startCode">Từ:</Label>
              <Input
                id="startCode"
                value={startCode}
                onChange={(e) => handleCodeChange(e.target.value, setStartCode)}
                placeholder="100000"
                maxLength={6}
              />
              {startCode && !validateCode(startCode) && (
                <p className="mt-1 text-xs text-red-500">Mã phải là 6 chữ số</p>
              )}
            </div>
            <div className="space-y-2">
              <Label htmlFor="endCode">Đến:</Label>
              <Input
                id="endCode"
                value={endCode}
